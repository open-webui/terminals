...
class PolicyLifecycleState(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "policy_id",
            "context_id",
            name="uq_policy_lifecycle_states_user_policy_context",
        ),
    )
    ...
    policy_id = Column(String, nullable=False, index=True)
    context_id = Column(String, nullable=False, default="default", index=True)
    data = Column(JSON, nullable=False, default=dict)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"policy_id={self.policy_id!r} context_id={self.context_id!r}>"
        )
